...
export function Pagination({ page, value }: { page: number; value: string }) {
  const router = useRouter();
  const handleOnNext = () => {
    router.push(`/searchPage?value=${value}&page=${Page + 1}`);
  };
  const handleOnPrevious = () => {
    router.push(`/searchPage?value=${value}&page=${Page - 1 }`);
  };
  const handleOnDynamic = (Page: number) => {
   router.push(`/searchPage?value=${value}&page=${Page}`);
  };
